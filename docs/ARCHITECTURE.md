...
Before dry-run, Doctor, or real installation, [`checkExternalToolProvenance()`](../src/provenance.ts) compares selected lock-backed overrides with their package, repository, or release identity and rejects mutable values by default. Alternate lock files, re-identified locked sources, mutable locked-source versions, and a non-default Antigravity installer URL require the explicit mutable-source override. This opt-in changes provenance policy for that invocation rather than modifying the catalog.
