...
Agent Toolkit writes a versioned manifest for installed skills. The manifest records its scope and generation time, plus each skill's runtime, source, absolute destination, and installation time. Local and global scopes use separate manifests.

Repair re-runs the selected installers. Successful skill installs update or insert their manifest entries and refresh the manifest generation time:
...
Uninstall removes only manifest-recorded skill destinations that are direct, contained children of recognized runtime skill roots. It does not sweep unrecorded files or remove arbitrary directories. If a destination escapes its expected root, changes identity during removal, or otherwise fails safety validation, removal is rejected and the manifest is preserved for review or retry.
