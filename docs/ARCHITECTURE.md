...
Agent Toolkit is a layered Node.js command-line installer that turns command-line arguments, interactive answers, environment-backed defaults, and a validated catalog of lock-backed external sources into a resolved set of tools, runtimes, and install scope. A central orchestrator validates applicable provenance and prerequisites, delegates work to tool-specific adapters, installs supported skills into runtime-specific targets, records the Custom Skill paths it manages, and reports human-readable status with JSON available for supported read-only operations. The project is intentionally adapter-based: selection and policy remain in shared modules, while installers own the commands required by each external tool.
...
| [`skills.ts`](../src/skills.ts) | Discover, filter, validate, install, and record Custom Skills. |
...
[`tools.lock.json`](../tools.lock.json) is the source of truth for the external identities represented in it: RTK, Caveman, Graphify, GSD, Agent Browser, the Agent Skills CLI and repositories, and npm-installed runtime CLIs. [`loadToolLock()`](../src/tool-lock.ts) rejects unsupported schema versions, mutable package versions, non-full Git refs, invalid SHA-256 values, unknown bundle IDs, unsafe relative catalog paths, and bundle entries that reference unknown repositories. The Agent Skills section separates repository identity from bundle metadata: repository records pin upstream projects to full commits, while bundle records provide the label, description, skill name, and optional repository-relative path. Status derives bundle counts and source details from the catalog; [`installAgentSkillBundle()`](../src/installers/agent-skills.ts) consumes the metadata and entries to perform installation.

Before dry-run, Doctor, or real installation, [`checkExternalToolProvenance()`](../src/provenance.ts) applies source-specific gates. Package and Git specifications must retain the locked package or repository identity and use an immutable version or full commit. RTK uses a different contract: by default its release-metadata URL must stay below the locked repository's `/releases/` namespace and must not use `/releases/latest`, but the URL is not required to name the exact `tools.rtk.tag`; another tagged-release URL in the same repository can pass this gate. [`installers/rtk.ts`](../src/installers/rtk.ts) anchors the selected archive separately by verifying its bytes against the locked per-asset SHA-256 before extraction. Alternate lock files, re-identified locked sources, mutable locked-source versions, RTK URLs outside that boundary, and a non-default Antigravity installer URL require the explicit mutable-source override. This opt-in changes provenance policy for that invocation rather than modifying the catalog.

Two installer paths sit outside content-pinned lock guarantees. [`installers/superpowers.ts`](../src/installers/superpowers.ts) delegates to runtime-native plugin or extension systems using the unversioned identifiers `superpowers@claude-plugins-official` and `superpowers@openai-curated`, or the Gemini extension URL `https://github.com/obra/superpowers`; Agent Toolkit does not pin the content those identifiers resolve to in `tools.lock.json`. The official Antigravity install script is also outside the lock: [`src/provenance.ts`](../src/provenance.ts) requires HTTPS without embedded credentials and identity-gates overrides against the official URL, but it does not pin the script content by checksum or commit. These exceptions do not weaken validation for sources that are represented in the lock.
...
The versioned manifest tracks only Custom Skills installed through [`src/skills.ts`](../src/skills.ts), including Custom Skills selected from an alternate `--skills-dir` source. It does not track external Agent Skills bundles, tool packages, runtime CLIs, plugins, extensions, or RTK binaries. Each entry records a runtime, source, absolute destination, and installation timestamp; local and global scopes use separate manifests below the project or user `.agent-toolkit/` directory.

Toolkit-managed Custom Skill copies use a sibling temporary directory and rename for each destination. This path covers non-Gemini runtimes and the Gemini fallback used when Gemini CLI is unavailable. When Gemini CLI is present, the toolkit instead delegates to `gemini skills install`; after that command succeeds, it records the expected destination below the resolved Gemini skill root even though that destination did not pass through `copySkillDir()` staging. Manifest entries from both paths are upserted by absolute destination and held in memory until dispatch finishes. Manifest persistence writes formatted JSON to a sibling `.tmp` file and renames it over the manifest, so consumers do not observe a partially written manifest document.

Doctor is read-only: it combines current detection with the resolved selection and reports missing tools, runtimes, or skills. Repair re-runs selected installers; only successful Custom Skill installations handled by the Custom Skills installer, whether toolkit-copied or Gemini-delegated, refresh managed entries. Uninstall reads the recorded paths instead of scanning runtime directories. Before removal, it validates the complete manifest, selected runtimes, current scope, recognized skill roots, direct-child relationship, canonical containment, and filesystem identity. A separate Node subprocess rechecks root and destination identity, renames the destination to a same-root random tombstone, rechecks containment, and only then removes it. A missing recorded destination can be pruned from the manifest, while an identity or containment failure preserves the manifest for manual review or retry.
...
- [`src/provenance.ts`](../src/provenance.ts) gates selected lock-backed state values and the Antigravity installer URL before side effects, within the pinning limits described above.
...
- [`src/skills.ts`](../src/skills.ts) prevents directory creation through a file path and stages toolkit-managed Custom Skill copies before replacement; Gemini CLI-delegated installs remain inside Gemini's own write boundary.
